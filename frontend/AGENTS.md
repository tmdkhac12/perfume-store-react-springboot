...
1. Xác định trước khu vực bị ảnh hưởng (Public, User, hoặc Admin) và liệt kê chính xác các file trước khi chỉnh sửa.
1. Tái sử dụng token Tailwind, typography, spacing và pattern tương tác hiện có từ [index.html](index.html) và trang gần nhất liên quan.
1. Triển khai bộ thay đổi nhỏ nhất nhưng an toàn; tránh refactor không liên quan.
1. Sau mỗi thay đổi lớn, chạy vòng kiểm tra chéo:
...
1. Nếu phát hiện sai lệch, tiếp tục tinh chỉnh cho đến khi kết quả khớp hoàn toàn với yêu cầu và quy ước dự án.
1. Khi bàn giao phiên làm việc, ghi rõ đã hoàn thành gì, bước tiếp theo là gì, và các quyết định quan trọng trong file này hoặc trong ghi chú dự án được tham chiếu.
