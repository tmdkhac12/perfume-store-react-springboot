...
/** @description: Cart item row displaying product details and quantity. */
function CartItem({ item, onDecrease, onIncrease, onRemove }) {
  const volumeLabel = item?.concentration ? `${item.concentration} - ${item.volume}` : item.volume;
    ...
    <div className="flex flex-col sm:flex-row gap-8 items-start relative group">
      ...
      <div className="flex flex-col flex-grow w-full">
        <div className="flex justify-between items-start w-full">
          ...
          <button
            className="text-on-surface-variant hover:text-error transition-colors rounded-full"
            type="button"
            onClick={onRemove}
            disabled={!onRemove}
          >
        ...
        <div className="flex justify-between items-end mt-auto pt-4">
          <div className="flex items-center border-b border-outline-variant/30 pb-1">
            <button
              className="text-on-surface-variant hover:text-on-background px-2 text-lg leading-none rounded-full"
              type="button"
              onClick={onDecrease}
              disabled={!onDecrease}
            >
