...
function ShopFilters({
  ...
  return (
    <aside className="w-full hidden lg:block lg:w-72 flex-shrink-0 bg-surface-container-low p-8 lg:top-[160px] rounded-[2rem]">
      <div className="mb-12">
        <h3 className="font-label text-xs uppercase tracking-[0.2em] text-primary mb-6 border-b border-outline-variant/30 pb-3">SEARCH</h3>
        <input
        ...
        <div className="flex flex-col gap-4">
          {genderOptions.map((option) => {
            ...
            return (
              ...
              >
                <span className={`w-4 h-4 border flex items-center justify-center transition-colors rounded-full ${isActive ? 'border-primary' : 'border-outline-variant group-hover:border-primary'}`}>
                  <span className={`w-2 h-2 rounded-full ${isActive ? 'bg-primary' : ''}`}></span>
                </span>
                <span className={`font-body text-sm transition-colors ${isActive ? 'text-primary' : 'text-on-surface-variant group-hover:text-primary'}`}>
                  {option.label}
                </span>
              </button>
            );
      ...
      <div className="mb-12">
        ...
        <div className="flex flex-col gap-4">
          ...
          {brandStatus === 'error' && (
            <span className="font-body text-sm text-on-surface-variant">{brandError || 'Unable to load brands.'}</span>
          )}
      ...
      <div className="mb-12">
        ...
        <div className="flex flex-col gap-4">
          ...
          <button className="w-full py-3 text-center bg-primary text-white font-label text-[10px] uppercase tracking-[0.2em] hover:bg-opacity-80 transition-all rounded-[2rem]" type="button" onClick={onApplyFilters}>
            APPLY
          </button>
        </div>
      </div>
      <button className="w-full py-4 text-center border border-outline-variant/30 text-on-surface font-label text-xs uppercase tracking-[0.2em] hover:bg-surface-container-high transition-colors mt-4 rounded-[2rem]" type="button" onClick={onClearFilters}>
        CLEAR FILTERS
      </button>
    </aside>
  );
