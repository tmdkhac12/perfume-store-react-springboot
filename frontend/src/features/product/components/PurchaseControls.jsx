...
function PurchaseControls({ onAddToCart }) {
  ...
  /**
   * @description: Sends the selected quantity to the parent add-to-cart handler.
   * @flow: Click add -> Forward quantity -> Parent updates cart.
   */
  const handleAddToCart = () => {
    onAddToCart?.(quantity);
  };

  const isAddDisabled = !onAddToCart;
  ...
  return (
    <div className="flex flex-col gap-4 mt-auto">
      <div className="flex gap-4">
        ...
        <button
          className="flex-1 bg-gradient-to-b from-primary to-primary-container text-on-primary py-5 font-label text-xs uppercase tracking-[0.2em] hover:bg-secondary transition-colors duration-300 rounded-full disabled:cursor-not-allowed disabled:opacity-60"
          type="button"
